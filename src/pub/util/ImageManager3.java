...
/**
 * 图片加载类
 * 0822用在动态列表用户\动态详情的用户\评论动态用户头像，显示为圆形图片，如果没有网络上来的图片，就显示默认的图片
 */
public class ImageManager3 {

	private static ImageManager3 imageManager;
	 ...
	 * @return
	 */
	public static ImageManager3 from(Context context) {
		mContext = context;
		// 如果不在ui线程中，则抛出异常
		...
		if (imageManager == null) {
			imageManager = new ImageManager3(myapp);
		}
...
//				//ZZK
				Bitmap bmBitmap = UtilsZZK.toRoundBitmap(BitmapFactory.decodeResource(mContext.getResources(), resId));
				if(bmBitmap != null)
		...
		if (bitmap != null) {
			...
			//ZZK
			Bitmap bmBitmap = UtilsZZK.toRoundBitmap(bitmap);
			if(bmBitmap != null)
				setImageBitmap(imageView, bmBitmap, false);
			return;
		}
...
//				//ZZK
		...
		queueNewImage(new ImageRef(imageView, url, filePath, resId));
	}
...
//				//ZZK
		...
		queueNewImage(new ImageRef(imageView, url, filePath, resId, width,
				height));
	}
		...
		public void handleMessage(Message msg) {
			...
			case MSG_REQUEST: // 收到请求
				...
				if (mImageManagerHandler != null) {
					...
					if(bitmap == null){
						message = mImageManagerHandler.obtainMessage(
								MSG_REPLY, bitmap);
					}else{
						Bitmap bmBitmap = UtilsZZK.toRoundBitmap(bitmap);
						message = mImageManagerHandler.obtainMessage(
			...
			case MSG_REFRESH_ONE:
				...
				if (mImageManagerHandler != null) {
					...
					if(bitmap1 == null){
						...
						Bitmap bitmapaaa = UtilsZZK.toRoundBitmap(bitmap1);
						message = mImageManagerHandler.obtainMessage(
								MSG_REPLY, bitmapaaa);
					}
