...
public class MainActivityHX extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.hx_activity_main);
		initView();
	...
	public int getUnreadAddressCountTotal() {
		...
		if (SEMapApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME) != null)
			unreadAddressCountTotal = SEMapApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME).getUnreadMsgCount();
...
//			Toast.makeText(getBaseContext(), "from: "+from, 1).show();
			// 消息id
...
//			message.getStringAttribute("");
			...
			Toast.makeText(MainActivityHX.this, "收到透传：action："+action, Toast.LENGTH_SHORT).show();
		}
	};
	...
	private class MyContactListener implements EMContactListener {
		...
		public void onContactAdded(List<String> usernameList) {
			// 保存增加的联系人
			Map<String, User> localUsers = SEMapApplication.getInstance().getContactList();
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					if (ChatActivity.activityInstance != null && usernameList.contains(ChatActivity.activityInstance.getToChatUsername())) {
						Toast.makeText(MainActivityHX.this, ChatActivity.activityInstance.getToChatUsername() + "已把你从他好友列表里移除", 1).show();
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					if (error == EMError.CONNECTION_CONFLICT) {
						...
						if (NetUtils.hasNetwork(MainActivityHX.this))
		...
		public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					if (currentTabIndex == 0)
						chatHistoryFragment.refresh();
					if (CommonUtils.getTopActivity(MainActivityHX.this).equals(GroupsActivity.class.getName())) {
...
//	}
	...
	private void showConflictDialog() {
		...
		SEMapApplication.getInstance().logout(null);

		if (!MainActivityHX.this.isFinishing()) {
			// clear up global variables
			try {
				if (conflictBuilder == null)
					conflictBuilder = new android.app.AlertDialog.Builder(MainActivityHX.this);
				...
				conflictBuilder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						conflictBuilder = null;
						finish();
						startActivity(new Intent(MainActivityHX.this, LoginActivity.class));
					}
				});
