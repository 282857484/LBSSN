...
import pri.z.main.Fragment_Activity;
import pri.z.main.Fragment_Moment;
import pri.z.main.MainActivity;
import pri.z.main.MainLeftMenu;
 ...
 *2014-3-8
 */
public abstract class sqlSecurityThread  implements Runnable {
//	private DBAdapter(){}
	...
	protected static ConcurrentLinkedQueue<SQLInfo> SQLWaitQueue = new ConcurrentLinkedQueue<SQLInfo>();
...
//	public  SQLiteSecurityThread(Context _context) {
//	    context = _context;
//	  }
	 ...
	 * @param sqlinfo
	 */
	public static void handleSQl(SQLInfo sqlinfo) {
		SQLWaitQueue.add(sqlinfo);
	}

	private static final boolean runSQL = true;

	@Override
	public void run() {

		while (runSQL) {
			if (!SQLWaitQueue.isEmpty()) {
				SQLResponse sqlr = handleSQLNow(SQLWaitQueue.remove());
				messenger = SEMapApplication.currentMessenger;
				if(sqlr.p == SQLiteProtocol.queryActivityData){
					messenger = Fragment_Activity.myMessenger;
				}else if(sqlr.p == SQLiteProtocol.queryMomentData){
					messenger = Fragment_Moment.myMessenger;
				}else if(sqlr.p == SQLiteProtocol.queryAllSearchDistancesInMainActivity){
					messenger = MainActivity.myMessenger;
				}else if(sqlr.p == SQLiteProtocol.queryUserInMainLeft){
					messenger = MainLeftMenu.myMessenger;
				}
				// 必须在activity中指定currentMessager
				// 值可以返回到activity   
				...
				if(messenger == null) {
					messenger = SEMapApplication.serviceMessenger;
					try {
						messenger.send(msg);
	...
	/**
	 * 这里只是个样例 返回的值会到handle中取出来
	 * 
	 * @param sqlinfo
	 * @return
	 */
	protected SQLResponse handleSQLNow(SQLInfo sqlinfo) {

		SQLResponse response = new SQLResponse();
		response.p = sqlinfo.p;

		switch (sqlinfo.p) {
		// 增为同一操作
		case SQLiteProtocol.insertCommonData:
			// 执行添加操作
//			Log.e("哈哈哈哈", "sqlSecurityThread insertCommonData");
			response.mark = handleINSERT((InsertObject) sqlinfo.SQLInfo);
//			if(response.mark != -1){
//				Log.e("哈哈哈哈", "Insert Success !  response.mark  is:  "+response.mark);
//			}
			break;

		case SQLiteProtocol.deleteCommonData:
			// 执行删除操作
//			Log.e("哈哈哈哈", "sqlSecurityThread deleteCommonData");
			response.mark = handleDELETE((DeleteObject) sqlinfo.SQLInfo);
//			Log.e("哈哈哈哈", "Delete response.mark  is:  "+response.mark);
//			if(response.mark > 0){
//				Log.e("哈哈哈哈", "Delete Success !  response.mark  is:  "+response.mark);
//			}
			break;
		...
		case SQLiteProtocol.queryUserInMainLeft:
			// 执行查询操作
//			Log.e("哈哈哈哈", "sqlSecurityThread queryUserInfoSelectData");
			response.result = handleQUERY((QueryObject) sqlinfo.SQLInfo);
			if (response.result == null) {
//				Log.e("哈哈哈哈", "sqlSecurityThread response.resultSet == null ");
				response.mark = 2;
			} else {
//				Log.e("哈哈哈哈", "sqlSecurityThread response.resultSet Has Data");
				response.mark = 1;
			}
			break;

		case SQLiteProtocol.updateCommonData:
			// 执行更新操作
//			Log.e("哈哈哈哈", "sqlSecurityThread updateCommonData");
			response.mark = handleUPDATE((UpdateObject) sqlinfo.SQLInfo);
//			if(response.mark > 0){
//				Log.e("哈哈哈哈", "Update Success !  response.mark  is:  "+response.mark);
//			}
			break;

		default:
			Log.e("哈哈哈哈", "Unknown SQL Protocol");
		}
		return response;
	}
	
	/**
	 * 需要根据相应的数据库作调整 UpdateObject QueryObject DeleteObject InsertObject 是必须的参数而已
	 * 
	 * @return
	 */
	protected abstract long handleUPDATE(UpdateObject updateObj);

	protected abstract Object handleQUERY(QueryObject queryObj);

	protected abstract long handleDELETE(DeleteObject deleteObj);

	protected abstract long handleINSERT(InsertObject insertObj);
	
	public static sqlSecurityThread dbAdapter = null;
